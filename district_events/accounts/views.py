...
def about(request):
    try:
        response = requests.get('http://localhost:5000/api/about')
...
def contact(request):
    ...
    try:
        response = requests.get('http://localhost:5000/api/contact')
        contact_info = response.json() if response.status_code == 200 else None
